...
describe("resolveReactRouterConfig", () => {
  it("applies React Router's defaults", async ({ cacheDir }) => {
  ...
  it("assembles the route manifest nested under the root route", async ({ cacheDir }) => {
  ...
  it("returns a deeply frozen config", async ({ cacheDir }) => {
    const resolved = await resolveReactRouterConfig(fixtureRoot("minimal"), { cacheDir });
  ...
  it("honors appDirectory from react-router.config and resolves fs-routes", async ({
    cacheDir,
  }) => {
  ...
  it("resolves config only, with an empty manifest, when skipRoutes is set", async ({
    cacheDir,
  }) => {
    const root = fixtureRoot("minimal");
  ...
  it("loads the project's vite.config so import aliases in routes.ts resolve", async ({
    cacheDir,
  }) => {
  ...
  it("expands prefix() into per-entry paths", async ({ cacheDir }) => {
  ...
  it("preserves splat segments", async ({ cacheDir }) => {
    const resolved = await resolveReactRouterConfig(fixtureRoot("splat"), { cacheDir });

    expect(resolved.routes["files"]?.path).toBe("files/*");
  });

  it("keeps React Router's absolute id from the relative() helper while normalizing file to app-relative", async ({
    cacheDir,
  }) => {
  ...
  it("uses an explicit route id when provided", async ({ cacheDir }) => {
  ...
  it("propagates RouteValidationError for the reserved 'root' id", async ({ cacheDir }) => {
    await expect(
      resolveReactRouterConfig(fixtureRoot("root-id"), { cacheDir }),
    ).rejects.toBeInstanceOf(RouteValidationError);
  });

  it("propagates RouteEvaluationError when routes.ts is missing", async ({ cacheDir }) => {
    await expect(
